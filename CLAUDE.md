...
- **Reste au smoke seulement** (pas d'exercice fonctionnel profond) : le **fly-to 3D** n'a pas été déclenché sur une vraie recherche (garage synthétique sans meuble) — code guardé try/catch ; et le **panneau extension** (pas un composant App, se teste en rechargeant l'extension). Faible risque.
