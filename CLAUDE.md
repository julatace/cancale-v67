...
- **Écran « Comptes liés »** (`VintedAccounts`, accessible depuis Paramètres) : **gestion des comptes uniquement** — lister les comptes captés par l'extension, les renommer (`vinted_account_labels`), tester la connexion, actualiser. **Plus aucune vue de données ici** (tout est dans les onglets dédiés ci-dessus) — évite les doublons.
