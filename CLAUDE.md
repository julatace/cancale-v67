...
- **Refresh centralisé + cron (juillet 2026) — accès permanent sans reconnexion.** Endpoint `api/vinted-refresh.js` : lit tous les comptes dans Supabase et rafraîchit **uniquement** ceux dont l'access_token expire (décodage de l'`exp` du JWT, marge 15 min), puis persiste. Appelé (a) au démarrage de l'app (`fetch('/api/vinted-refresh')` dans un `useEffect` de `App`), (b) par un **cron Vercel** (`vercel.json`, `0 4 * * *`) pour garder la session vivante même app fermée. Le refresh centralisé + le décodage d'exp limitent les rotations inutiles et les courses entre appareils. C'est ce qui permet à Julien de consulter ses comptes depuis n'importe quel appareil sans se reconnecter. **Limite honnête** : si Vinted invalide le refresh_token (sécurité, très longue inactivité), il faut que l'extension recapture une session (repasser sur vinted.fr une fois) — aucun tiers ne peut éviter ce cas.
