...
function getStockCatColor(cat){
  const map={Vinted:()=>C.accent,Garage:()=>C.warn,Compta:()=>C.blue};
  if(map[cat]) return map[cat]();
  const h=[...String(cat)].reduce((a,c)=>a*31+c.charCodeAt(0)|0,0);
  const opts=[()=>C.purple,()=>C.danger,'#e07b3d','#6abf6a'];
              ...
              return (
                <div key={`c${zi}_${ci}`} style={{display:'flex',flexDirection:'column',alignItems:'flex-start',gap:GAP}}>
                  ...
                  {arr.map((val,si)=>{
                    ...
                    if(!showAllCells&&!blocked&&t===''&&!displayColor){
