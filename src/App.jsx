...
const platBadge=(acc)=>{
  ...
  return {short:pl.short,color:pl.color,label:pl.label};
};
        ...
        <div style={{background:'#F56B2A',borderRadius:10,padding:'10px 14px',display:'flex',flexDirection:'column',gap:6}}>
          ...
          {crossAlert.map((x,i)=>(
            <div key={i} style={{display:'flex',alignItems:'center',gap:8}}>
              <span style={{background:'rgba(0,0,0,0.2)',color:'#fff',fontWeight:900,fontSize:11,padding:'2px 8px',borderRadius:12,border:'1px solid rgba(255,255,255,0.4)'}}>{x.short} {x.pl}</span>
              <span style={{color:'#fff',fontSize:12}}>#{x.id} — {x.acc}</span>
            </div>
          ))}
