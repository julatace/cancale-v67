...
function hexShade(hex,pct){
  ...
  return '#'+(nr*65536+ng*256+nb).toString(16).padStart(6,'0');
}
const PLATFORMS={
  vinted:{label:'Vinted',short:'V',color:'#09B1BA'},
  leboncoin:{label:'Leboncoin',short:'L',color:'#F56B2A'},
  ebay:{label:'eBay',short:'E',color:'#E53238'},
};
const platBadge=(acc)=>{
  if(!acc) return null;
  const pl=PLATFORMS[acc.platform||'vinted']||PLATFORMS.vinted;
  // Abbreviate: "Compte 1" → "C1", "Leboncoin Pro" → "Lb", "eBay Shop" → "eB"
  const words=(acc.name||pl.label).trim().split(/\s+/);
  let abbr;
  if(words.length===1){abbr=words[0].slice(0,3);}
  else{
    // If second word is a number: "Compte 1" → "C1"
    if(/^\d+$/.test(words[words.length-1])){abbr=words[0][0]+words[words.length-1];}
    else{abbr=words[0].slice(0,2);}
  }
  return {short:abbr,color:acc.color||pl.color,label:pl.label,platform:pl.label};
};
...
function AccountsSettings({accounts,setAccounts,appsScriptUrl,setAppsScriptUrl,dark,toggleDark,notifEnabled,onToggleNotif,onExport,onImport}) {
  ...
  const addAcc=()=>{
    ...
    const newAcc={id:'acc'+Date.now(),name:`Compte ${accounts.length+1}`,color,platform:'vinted',email:'',pseudo:'',phone:''};
  ...
  return (
    <div style={{padding:16,display:'flex',flexDirection:'column',gap:14}}>
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        <h2 style={{margin:0,color:C.accent,fontSize:20,fontWeight:800}}>Comptes</h2>
        ...
        {accounts.map((acc,i)=>(
          <div key={acc.id} style={{display:'flex',flexDirection:'column',gap:6,padding:'10px 12px',background:C.bg,borderRadius:8,border:`1px solid ${acc.color}44`}}>
            ...
            <div style={{display:'flex',gap:6,alignItems:'center',flexWrap:'wrap'}}>
              <span style={{fontSize:11,color:C.muted,minWidth:80}}>Plateforme :</span>
              {Object.entries(PLATFORMS).map(([k,pl])=>(
                <button key={k} type="button" onClick={()=>upd(i,'platform',k)} style={{
                  padding:'3px 10px',borderRadius:20,fontSize:11,fontWeight:700,cursor:'pointer',fontFamily:'inherit',
                  background:(acc.platform||'vinted')===k?pl.color:'transparent',
                  color:(acc.platform||'vinted')===k?'#fff':C.muted,
                  border:`1.5px solid ${(acc.platform||'vinted')===k?pl.color:C.border}`,
                }}>{pl.label}</button>
              ))}
            </div>
...
function Catalog({catalog,setCatalog,onDeleteId,accounts,photos,setPhotos}) {
  ...
  const [newRow,setNewRow]=useState(()=>{
    try{const s=localStorage.getItem('vinted_next_cat_id');if(s)return {id:s,buyPrice:'',account:'',listings:[]};} catch(_){}
    const nums=(catalog||[]).map(p=>parseInt(p.id,10)).filter(n=>!isNaN(n));
    const nextId=nums.length?String(Math.max(...nums)+1):'';
    return {id:nextId,buyPrice:'',account:'',listings:[]};
  });
  ...
  const addRow=()=>{
    ...
    const u=[...catalog,{id,buyPrice:+newRow.buyPrice,status:'stock',addedAt:tod(),...(newRow.account?{account:newRow.account}:{}),listings:newRow.listings||[]}];
    setCatalog(u); save('vinted_catalog',u);
    setLastAddedId(id);
    let nextId='';
    if(/^\d+$/.test(id)) nextId=String(parseInt(id,10)+1);
    setNewRow({id:nextId,buyPrice:'',account:newRow.account,listings:newRow.listings||[]});
                  ...
                  <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:3}}>
                    {(()=>{
                      ...
                      return (<>
                        ...
                        {/* Platform badges — which accounts this item is listed on */}
                        {(p.listings||[]).length>0&&(
                          <div style={{display:'flex',gap:2,flexWrap:'wrap',justifyContent:'center',marginTop:2}}>
                            {(p.listings||[]).map(lid=>{
                              const acc=accounts&&accounts.find(a=>a.id===lid);
                              if(!acc) return null;
                              const pb=platBadge(acc);
                              return <span key={lid} title={`${pb.label} — ${acc.name}`} style={{fontSize:9,fontWeight:900,padding:'1px 4px',borderRadius:3,background:pb.color,color:'#fff',cursor:'pointer'}}
                                onClick={()=>{const ls=(p.listings||[]).filter(x=>x!==lid);update(p.id,'listings',ls);}}
                              >{pb.short}</span>;
                            })}
                          </div>
                        )}
...
function Sales({catalog,setCatalog,sales,setSales,invoices,invoiceSettings,accounts}) {
  ...
  const [crossAlert,setCrossAlert]=useState(null); // {items:[{id,acc,pl}]}
  ...
  const addRow=()=>{
    ...
    const crossItems=[];
    foundItems.forEach(ci=>{
      (ci.listings||[]).forEach(lid=>{
        const acc=accounts&&accounts.find(a=>a.id===lid);
        if(acc){const pb=platBadge(acc);crossItems.push({id:ci.id,acc:acc.name,pl:pb.label,color:pb.color,short:pb.short});}
      });
    });
    if(crossItems.length>0) setCrossAlert(crossItems);
  ...
  return (
    <div style={{padding:16,display:'flex',flexDirection:'column',gap:14}}>
      {/* Cross-platform alert */}
      {crossAlert&&crossAlert.length>0&&(
        <div style={{background:'#F56B2A',borderRadius:10,padding:'10px 14px',display:'flex',flexDirection:'column',gap:6}}>
          <div style={{color:'#fff',fontWeight:800,fontSize:13}}>📦 Article(s) vendu(s) — pense à retirer sur ces plateformes :</div>
          {crossAlert.map((x,i)=>(
            <div key={i} style={{display:'flex',alignItems:'center',gap:8}}>
              <span style={{background:x.color,color:'#fff',fontWeight:900,fontSize:11,padding:'2px 8px',borderRadius:12}}>{x.short}</span>
              <span style={{color:'#fff',fontSize:12}}>#{x.id} — retirer de <b>{x.acc}</b> ({x.pl})</span>
            </div>
          ))}
...
/* ── Box & Door ──────────────────────────────────────── */
const Box = React.memo(function Box({val,isSold,highlight,accountColor}) {
  ...
  else if(accountColor&&hasVal) {front=accountColor;side=hexShade(accountColor,-0.35);top=hexShade(accountColor,0.25);textCol='#fff';}
              ...
              return (
                <div key={`c${zi}_${ci}`} style={{display:'flex',flexDirection:'column',alignItems:'flex-start',gap:GAP}}>
                  ...
                  {arr.map((val,si)=>{
                    ...
                    const accountColor=!cellColor&&t?numColors[t]:null;
                    ...
                    return (
                      <div key={si} ref={highlight?highlightRef:null} onClick={()=>{
                        ...
                        {t&&numListings[t]&&numListings[t].length>0&&(
                          <div style={{position:'absolute',bottom:1,left:0,right:0,display:'flex',justifyContent:'center',gap:1,zIndex:4,pointerEvents:'none'}}>
                            {numListings[t].map(lid=>{
                              const acc=accounts&&accounts.find(a=>a.id===lid);
                              if(!acc) return null;
                              const pb=platBadge(acc);
                              return <span key={lid} style={{fontSize:5.5,fontWeight:900,padding:'0 2px',borderRadius:2,background:pb.color,color:'#fff',lineHeight:'8px',letterSpacing:0}}>{pb.short}</span>;
                            })}
                          </div>
                        )}
        ...
        Object.entries(grouped).map(([cat,entries])=>(
          <div key={cat}>
            ...
            <div style={{display:'flex',flexWrap:'wrap',gap:7}}>
              {entries.map(x=>{
                const catItem=catalog&&catalog.find(p=>String(p.id)===String(x.num));
                const itemListings=catItem&&catItem.listings&&catItem.listings.length?catItem.listings:[];
                return (
                  <span key={x.num+x.cat} style={{display:'inline-flex',alignItems:'center',gap:4,background:C.card,border:`1px solid ${C.border}`,borderRadius:8,padding:'5px 8px 5px 11px',fontSize:13,fontWeight:700,color:C.text}}>
                    {itemListings.map(lid=>{
                      const acc=accounts&&accounts.find(a=>a.id===lid);
                      if(!acc) return null;
                      const pb=platBadge(acc);
                      return <span key={lid} title={`${pb.label} — ${acc.name}`} style={{fontSize:10,fontWeight:900,padding:'1px 4px',borderRadius:3,background:pb.color,color:'#fff'}}>{pb.short}</span>;
                    })}
                    {x.num}
