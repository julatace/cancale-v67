...
// Recupere les conversations (messages). Endpoint deduit par convention REST
// a partir de /api/v2/conversations/stats (confirme fonctionnel) - a affiner
// si les noms de champs ne correspondent pas a ce qu'on affiche.
const fetchVintedConversations = async (account, page = 1) => {
  const res = await vintedApiCall(account, `/api/v2/conversations?page=${page}&per_page=20`);
  if (!res.ok) return { ok: false, error: res.status || res.error, items: [], pagination: null, raw: res };
  const items = res.data?.conversations || res.data?.data || [];
  return {
    ok: true,
    items,
    pagination: res.data?.pagination || null,
                    ...
                    <>
                      {orders[acc.vinted_user_id].items.length===0 && (
                        <div style={{fontSize:13,color:C.muted}}>
                          Aucune conversation trouvée (ou le format de réponse diffère de ce qui était attendu — endpoint deviné, à confirmer).
                        </div>
                      )}
                      <div style={{display:'flex',flexDirection:'column',gap:8}}>
                          ...
                          <div key={conv.id || i} style={{display:'flex',gap:10,alignItems:'center',padding:'8px 10px',borderRadius:10,border:`1px solid ${C.border}`,background:C.surface}}>
                            ...
                            <div style={{flex:1,minWidth:0}}>
                              <div style={{fontSize:12,fontWeight:700,color:C.text,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>
                                {conv.title || conv.subject || conv.opposite_user?.login || conv.item?.title || `Conversation`}
                              </div>
                              <div style={{fontSize:11,color:C.muted,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>
                                {conv.last_message?.body || conv.snippet || conv.body || ''}
                              </div>
                            </div>
                            <div style={{fontSize:10,color:C.muted,flexShrink:0}}>
                              {(conv.updated_at || conv.last_message_at) ? new Date(conv.updated_at || conv.last_message_at).toLocaleDateString('fr-FR') : ''}
                            </div>
