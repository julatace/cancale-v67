  ...
  return (
    <div style={{padding:'20px 16px 8px'}}>
      <div style={{borderRadius:18,border:`1px solid ${C.border}`,background:C.card,padding:'22px 20px',boxShadow:'0 1px 3px rgba(0,0,0,.04)'}}>
        ...
        <div style={{fontSize:11.5,color:C.muted,textAlign:'center',marginTop:14,lineHeight:1.4,paddingTop:14,borderTop:`1px solid ${C.border}`}}>
    ...
    (async () => {
      ...
      const buildGrille = (w, d, ht, base, rows, cols, slots) => {
        ...
        for (let r = 0; r < nr; r++) for (let c = 0; c < nc; c++) {
          ...
          else { cellMesh = new THREE.Mesh(new THREE.BoxGeometry(bw, bh, bd), key === firstEmpty ? nextM : ghostM); cellMesh.receiveShadow = true; } // emplacement vide (bleu = prochaine place)
          ...
          {(FURN_TYPES[selItem.type] || {}).build === 'grille' && (() => {
            ...
            return (
              <div style={{ flex: '1 1 100%', display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap', marginBottom: 4, padding: '7px 9px', border: `1px dashed ${C.border}`, borderRadius: 9 }}>
                ...
                <button onClick={() => fillGridSeries(selItem)} title="Remplir toutes les places vides en croissant" style={{ border: `1px solid ${C.accent}`, borderRadius: 7, background: `${C.accent}14`, color: C.text, fontSize: 11, fontWeight: 800, padding: '5px 9px', cursor: 'pointer' }}>🔢 Remplir en série</button>
