...
const TABS=['journee','collectif','plat_vinted','plat_leboncoin','plat_ebay','plat_vestiaire','prixmarche','dashboard','cat_annonces','cat_ventes','cat_achats','cat_bord','cat_msg','garage','invoices','settings','vintedaccounts','catalog','sales','leboncoin','stockvinted'];
(async()=>{
  ...
  for(const [W,H,tag] of [[390,844,'390 px'],[1512,950,'1512 px']]){
    ...
    for(const t of TABS){
      ...
      // (React attrape l'exception). Mesuré le 7 septembre : un piège TDZ a tué
      // l'écran Achats et ce banc a répondu « rendu conforme ». C'est la
      // capture qui l'a vu — plus maintenant.
