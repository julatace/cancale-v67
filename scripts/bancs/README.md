...
| `ebay.cjs` | **l'assistant eBay EXÉCUTÉ** : `ebay.js` n'avait jamais tourné (`node --check` ne lit que la syntaxe). Il sert une fausse page de mise en vente avec le VRAI en-tête d'eBay (mesuré), charge le script avec un faux `chrome.runtime`, et vérifie que le panneau s'ouvre, que les 4 champs se remplissent, que **la barre de recherche d'eBay reste vide**, et que sur une page non reconnue le bandeau dit « aucun champ » au lieu de laisser croire que l'annonce est prête |
